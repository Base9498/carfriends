...
import com.carfriend.Util.FileHandler;
...
public class UserController {
    ...
    /***
     * 用户绑定车辆
     * @param params 绑定参数，即用户id，车辆称呼，车名
     * @param file 车辆图片，不必要
     * @return 绑定结果
     */
    ...
    public Result UserBindCar(@RequestParam Map<String, String> params,
        ...
        bind.setCarCall(params.get("call"));
        bind.setCarName(params.get("name"));
        String image = null;
        //TODO:添加图片？
        if (FileHandler.FileIsAvailable(file)) {
            try {
                image = FileHandler.transferFile(file, "User", home);
                bind.setCarAvatar(image);
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                bind.setCarAvatar(image);
